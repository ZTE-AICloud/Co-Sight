...
from app.cosight.agent.actor.general_actor.prompt import general_actor_execute_task_prompt_zh, \
    general_actor_execute_task_prompt, general_actor_system_prompt_zh, general_actor_system_prompt
from app.cosight.agent.actor.general_actor.tools import build_general_actor_functions
...
class GeneralActorAgent(BaseAgent):
    """本地执行器：使用代码/文件/搜索/网页/文档/图像等多工具完成步骤。"""

    AGENT_ID = "general_actor"
    DESCRIPTION = (
        "通用报告生成执行器。主要负责收集已有资料并生成 HTML 报告，"
    ...
    def __init__(
        ...
        all_functions = build_general_actor_functions(
            self.plan,
            self.work_space_path
        )
        if functions:
        ...
        if is_chinese:
            sys_prompt = general_actor_system_prompt_zh(self.work_space_path)
        else:
            sys_prompt = general_actor_system_prompt(self.work_space_path)
    ...
    def act(self, question, step_index):
        ...
        if is_chinese:
            task_prompt = general_actor_execute_task_prompt_zh(
                question, step_index, self.plan, self.work_space_path
            )
        else:
            task_prompt = general_actor_execute_task_prompt(
                question, step_index, self.plan, self.work_space_path
            )
