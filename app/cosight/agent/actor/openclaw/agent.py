...
class OpenclawAgent:
    ...
    AGENT_ID = "openclaw"
    DESCRIPTION = (
        "安排定时任务与创建技能的个人助手（OpenClaw）。负责安排定时任务、创建可执行技能、"
        "生成技能包或 HTML/脚本演示，以及对本地文件系统进行操作（遍历目录、读写文件、执行命令等）。"
        "当步骤涉及「创建技能」「安排定时任务」「生成 HTML/技能包」「整合生成完整的人物生平/专题 HTML 生成技能」"
        "或「编写可复用脚本/技能」时，应选择本执行器。"
    )
